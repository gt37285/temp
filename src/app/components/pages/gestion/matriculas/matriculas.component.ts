...
@Component({
  ...
  styleUrls: ['./matriculas.component.scss', '../gestion.component.scss']
})
export class MatriculasComponent implements OnInit {
  ...
  listarEstudiantes(id_curso: string) {
    ...
    this.service.listarMatriculasPendientes(id_curso)
      .subscribe(
        ((data: any) => {
          this.loading = false
  ...
  aceptar(matricula: matriculaModel) {
    ...
    Swal.fire({
      ...
      if (data.value) {
        this.loading = true
        this.service.aceptarMatricula(matricula, this.id_curso)
          .subscribe(
            ((data: any) => {
              Swal.fire({
  ...
  rechazar(matricula: matriculaModel) {
    ...
    Swal.fire({
      title: "Estas Seguro?",
      text: `Confirma que deseas rechazar la matricula`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
      ...
      if (data.value) {
        ...
        this.service.rechazarMatricula(this.id_curso, matricula._id)
          .subscribe(
            ((data: any) => {
              Swal.fire({
                title: 'Proceso completado',
